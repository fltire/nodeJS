<template>
    <div style="padding: 0 20px">
        <el-button v-if="$isPermissions('menu:add')" size="mini" type="primary" @click="addOrUpdate()" icon="el-icon-plus" style="margin-bottom:10px">新增</el-button>
        <el-table :data="dataList" style="width: 100%;margin-bottom: 20px;" :header-cell-style="{'background-color':'#f8f8f9'}"
            ...
            <el-table-column fixed="right" label="操作" width="150">
                <template slot-scope="scope">
                    <el-button v-if="$isPermissions('menu:upt')" @click="addOrUpdate(scope.row)"  type="text" size="small"><i id="del" class="el-icon-edit"></i> 修改</el-button>
                    <el-button type="text" size="small" v-if="scope.row.son==='N'&&$isPermissions('menu:del')" @click="delMenu(scope.row.menuId)"><i id="del" class="el-icon-delete"></i> 删除</el-button>
                </template>
            </el-table-column>
        </el-table>
